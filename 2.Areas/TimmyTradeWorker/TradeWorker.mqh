...
void TradeWorker::reqManageTrade()
{
    ...
    for (int i = 0 ; i < OrdersTotal(); i++) {
        ...
        if (ObjectDescription(objId + tag_cPtBE) == "fa"){
            ...
            if (priceBE < priceEN && priceBE > priceTP) continue;
            // Fail signal
            if       (orderType == OP_BUY && Bid < priceBE && Bid < Low[1] && Close[1] < priceBE) priceEN += fabs(OrderCommission())/OrderLots() / gdLotSize;
            else if (orderType == OP_SELL && Bid > priceBE && Bid >High[1] && Close[1] > priceBE) priceEN -= fabs(OrderCommission())/OrderLots() / gdLotSize;
